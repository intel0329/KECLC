...
export const TableHeader = ({ label, children, className = "" }) => (
    <th className={`border border-gray-900 bg-black px-1 py-2 text-[13px] font-bold text-gray-300 text-center whitespace-nowrap ${className} `}>
        {children || label}
    </th>
);
...
export const SearchablePanelCell = ({ value, onChange, panels, className = "", placeholder = "", heightClass = "h-[48px]", activeDropdownId, onOpen, dropdownPos, loadId, excludeId, globalUsedPanelIds, localUsedIds, currentId }) => {
    ...
    const filteredPanels = useMemo(() => {
        ...
        return panels.filter(p => {
            ...
            // 1. 자기 자신 제외 (BANK self-reference 제외) - 순환 참조 방지를 위해 항상 제외
            if (id === excludeId) return false;

            // 2. 검색어가 없을 때만 적용되는 필터 (기본 드롭다운 목록용)
            if (search === '') {
                // 변압기 및 간선 계산서 제외
                const excludedPrefixes = ['transformer-', 'transformer_', 'panel-feeder-', 'low-voltage-receiving-'];
                if (excludedPrefixes.some(prefix => id.startsWith(prefix))) return false;

                // 현재 시트에서 이미 사용 중인 패널 제외
                if (localUsedIds && localUsedIds.includes(id)) return false;

                // 다른 시트에서 이미 사용 중인 패널 제외 (데이터 무결성)
                // globalUsedPanelIds는 Set 객체이므로 .has() 사용 (배열일 경우 대비하여 체크)
                if (globalUsedPanelIds && (globalUsedPanelIds.has ? globalUsedPanelIds.has(id) : globalUsedPanelIds.includes(id))) return false;
            }

            // 3. 검색 필터 (이름 포함 여부)
            return name.includes(search);
        });
    }, [panels, searchText, excludeId, localUsedIds, globalUsedPanelIds]);
