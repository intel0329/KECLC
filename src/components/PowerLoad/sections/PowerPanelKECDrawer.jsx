// src/components/PowerPanelKECDrawer.jsx
...
const PowerPanelKECDrawer = ({ isOpen, onClose, projectInfo, updateProjectInfo, onUpdateKecSettings, totalLoad, kecSettings, summaryStats, highlightSection, maxBranchAT }) => {
    ...
    const panelPhase = projectInfo?.phase || '3Φ-4W';

    // Load TCC 규약동작배율 table data
    const [tccData, setTccData] = useState(null);
    const [panelMiMultiplierType, setPanelMiMultiplierType] = useState(kecSettings?.atMiMultiplierType || 'delta2');
    const [mccK, setMccK] = useState(1.5);

    // Sync state when kecSettings changes (e.g., from another drawer)
    useEffect(() => {
        if (kecSettings?.atMiMultiplierType) {
            setPanelMiMultiplierType(kecSettings.atMiMultiplierType);
        }
    }, [kecSettings?.atMiMultiplierType]);
    useEffect(() => {
        const loadTccData = async () => {
            try {
                ...
                if (remoteData) {
                    setTccData(remoteData);
                } else {
                    const localData = localStorage.getItem('tccMultiplierData');
                    if (localData) setTccData(JSON.parse(localData));
                }

                // 2. MCC 설정 데이터 로드 (globalK 등)
                const mccDefaultKey = 'kelc_mcc_settings';
                const mccProjectKey = projectId !== 'GLOBAL' ? `${mccDefaultKey}_${projectId}` : mccDefaultKey;

                let mccData = await projectService.getRemoteData(mccProjectKey, projectId);
                if (!mccData && mccProjectKey !== mccDefaultKey) {
                    mccData = await projectService.getRemoteData(mccDefaultKey, projectId);
                }

                if (mccData) {
                    setMccK(Number(mccData.otherSettings?.globalK ?? mccData.globalK) || 1.5);
                } else {
                    const localMcc = localStorage.getItem(mccProjectKey) || localStorage.getItem(mccDefaultKey);
                    if (localMcc) {
                        try {
                            const parsed = JSON.parse(localMcc);
                            setMccK(Number(parsed.otherSettings?.globalK ?? parsed.globalK) || 1.5);
                        } catch (e) { }
                    }
                }
            } catch (e) {
                console.error("Failed to load TCC/MCC data in drawer", e);
            }
        };
        ...
        if (isOpen) {
            ...
            const handleUpdate = (e) => {
                if (e.type === 'storage') {
                    const projectId = projectService.getActiveProjectId() || 'GLOBAL';
                    const mccProjectKey = projectId !== 'GLOBAL' ? `kelc_mcc_settings_${projectId}` : 'kelc_mcc_settings';
                    if (e.key === 'tccMultiplierData' || e.key === 'kelc_mcc_settings' || e.key === mccProjectKey || e.key === 'kelc_mcc_settings_updated') {
                        loadTccData();
                    }
                } else {
                    loadTccData();
    ...
    // Panel IMS Calculation (Starting Current) - Pure Sum per User Request
    const panelIms = useMemo(() => {
        if (!summaryStats || !projectInfo) return 0;
        const v = panelPhase.includes('1Φ') ? 220 : 380;
        const sqrt3 = panelPhase.includes('1Φ') ? 1 : Math.sqrt(3);

        // IMS = (maxMotorKva + othersSumKva) * 1000 / (√3 * V)
        const pureKvaSum = summaryStats.maxMotorKva + summaryStats.othersSumKva;
        return (pureKvaSum * 1000) / (sqrt3 * v);
    }, [summaryStats, projectInfo, panelPhase]);
    ...
    const panelCircuit = useMemo(() => {
        ...
        return {
            ...
            p,
            cableDistance: projectInfo.branchDistance || 30,
    ...
    }, [projectInfo, totalLoad, panelPhase, panelIms, summaryStats.maxAbsoluteTm]);
    ...
    // AT_MS Panel-Level Calculation
    const panelAtMs = useMemo(() => {
        if (!summaryStats || !judgment?.at_b) return { status: '-', delta: '-', ims: 0, imsOverDelta: 0, recommendedIn: '-', alpha: 1.0, ims_raw: 0 };

        const alpha = Number(kecSettings?.marginFactors?.atms) || 1.0;
        const ims = panelIms;

        // δ lookup from TCC table
        const atbRecommendedIn = Number(judgment.at_b?.recommendedIn) || 0;
        const maxTm = summaryStats.maxTm || 0;

        let delta = 0;
        if (tccData && atbRecommendedIn > 0 && maxTm > 0) {
            // Use 3P table for 3-phase panels, 1P for single-phase
            const isThreePhase = !panelPhase.includes('1Φ');
            const tableData = isThreePhase ? tccData.threePhase : tccData.singlePhase;
            const headers = isThreePhase ? tccData.threePhaseHeaders : tccData.singlePhaseHeaders;

            if (tableData && headers) {
                // Find column: exact match first, then same-or-larger time
                let colIdx = -1;
                const headerValues = headers.map(h => Number(h) || 0);

                // Exact match first
                colIdx = headerValues.indexOf(maxTm);

                // If no exact match, find same-or-larger
                if (colIdx === -1) {
                    let closestIdx = -1;
                    let closestVal = Infinity;
                    headerValues.forEach((hv, i) => {
                        if (hv >= maxTm && hv < closestVal) {
                            closestVal = hv;
                            closestIdx = i;
                        }
                    });
                ...
                // Find row: exact AT match (exact match only per user requirement)
                if (colIdx >= 0 && tableData[atbRecommendedIn]) {
                    delta = Number(tableData[atbRecommendedIn][colIdx]) || 0;
                }
            }
        }
        ...
        let status = '-';
        let recommendedIn = '-';
        const in_breaker = Number(projectInfo?.mccbAT) || 0;

        if (delta > 0 && ims > 0) {
            imsOverDelta = (ims * alpha) / delta;
            status = (in_breaker >= imsOverDelta) ? 'Ok' : 'Fail';

            // Recommend: smallest CB AT >= imsOverDelta
            const getP = (ph) => {
                const cleanPh = String(ph || '').replace(/[-\s]/g, '').replace(/Ø/g, 'Φ').toUpperCase();
                if (cleanPh.includes('1Φ')) return 2;
                if (cleanPh.includes('3Φ3W')) return 3;
            ...
            const cbType = projectInfo?.mainBreakerType || 'MCCB';

            recommendedIn = CB_DATA
                .filter(row => row[0] === cbType && Number(row[2]) === cbPhase && Number(row[3]) === p)
                .map(row => Number(row[4]))
                .filter(at => at >= imsOverDelta)
                .sort((a, b) => a - b)[0] || 'N/A';
        } else if (summaryStats.maxMotorKva === 0) {
            // No motor loads — AT_MS is not applicable
            status = 'Ok';
        }

        return { status, delta: delta > 0 ? delta : '-', ims, imsOverDelta, recommendedIn, alpha, ims_raw: ims };
    }, [summaryStats, judgment, tccData, kecSettings, projectInfo, panelPhase]);

    // AT_MI Panel-Level Calculation
    const panelAtMi = useMemo(() => {
        if (!summaryStats || !judgment?.at_b) return { status: '-', multiplierValue: '-', imi: 0, imiOverM: 0, recommendedIn: '-', alpha: 1.0, imi_raw: 0, multiplierType: panelMiMultiplierType, k: mccK };

        const alpha = Number(kecSettings?.marginFactors?.atmi) || 1.0;
        const v = panelPhase.includes('1Φ') ? 220 : 380;
        const sqrt3 = panelPhase.includes('1Φ') ? 1 : Math.sqrt(3);

        // I_MI = (othersSumKva + maxMotorKva × K) × 1000 / (√3 × V)
        const imi = (summaryStats.othersSumKva + (summaryStats.maxMotorKva * mccK)) * 1000 / (sqrt3 * v);

        // Get multiplier value based on type
        let multiplierValue = 0;
        if (panelMiMultiplierType === 'delta2') {
            // Range-based δ₂ lookup from delta2Rows table
            const rows = tccData?.delta2Rows;
            const targetAt = Number(judgment.at_b?.recommendedIn) || 0;
            if (rows && Array.isArray(rows) && rows.length > 0 && targetAt > 0) {
                const sorted = rows
                ...
                multiplierValue = matched ? Number(matched.delta2) || 0 : (sorted.length > 0 ? Number(sorted[0].delta2) || 0 : 0);
            }
        } else {
            // δ from TCC table (same logic as AT_MS)
            const atbRecommendedIn = Number(judgment.at_b?.recommendedIn) || 0;
            const maxTm = summaryStats.maxTm || 0;
            ...
            if (tccData && atbRecommendedIn > 0 && maxTm > 0) {
                ...
                if (tableData && headers) {
                    ...
                    if (colIdx >= 0 && tableData[atbRecommendedIn]) {
                        multiplierValue = Number(tableData[atbRecommendedIn][colIdx]) || 0;
                    }
                }
            }
        }
        ...
        if (multiplierValue > 0 && imi > 0) {
            imiOverM = (imi * alpha) / multiplierValue;
            status = (in_breaker >= imiOverM) ? 'Ok' : 'Fail';

            // Recommend: smallest CB AT >= imiOverM
            const getP = (ph) => {
                const cleanPh = String(ph || '').replace(/[-\s]/g, '').replace(/Ø/g, 'Φ').toUpperCase();
                if (cleanPh.includes('1Φ')) return 2;
                if (cleanPh.includes('3Φ3W')) return 3;
            ...
            recommendedIn = CB_DATA
                .filter(row => row[0] === cbType && Number(row[2]) === cbPhase && Number(row[3]) === p)
                .map(row => Number(row[4]))
                .filter(at => at >= imiOverM)
                .sort((a, b) => a - b)[0] || 'N/A';
        } else if (summaryStats.maxMotorKva === 0) {
            status = 'Ok';
        }

        return { status, multiplierValue: multiplierValue > 0 ? multiplierValue : '-', imi, imiOverM, recommendedIn, alpha, imi_raw: imi, multiplierType: panelMiMultiplierType, k: mccK };
    }, [summaryStats, judgment, tccData, kecSettings, projectInfo, panelPhase, panelMiMultiplierType, mccK]);

    // --- AUTO-SYNC JUDGMENT RESULTS TO PROJECT-INFO ---
    // This allows Panel Feeder to access results that require internal load data
    useEffect(() => {
        if (!isOpen || !projectInfo || !panelAtMs || !panelAtMi || !judgment) return;

        const syncField = (key, value) => {
            if (projectInfo[key] !== value) {
                updateProjectInfo(key, value);
            }
        };

        // ATMS Caching
        syncField('cachedIms', panelAtMs.ims_raw ? panelAtMs.ims_raw.toFixed(2) : '');
        syncField('cachedDelta', panelAtMs.delta !== '-' ? String(panelAtMs.delta) : '');
        syncField('cachedAtms', panelAtMs.recommendedIn !== 'N/A' ? String(panelAtMs.recommendedIn) : '');
        syncField('cachedAtmsStatus', panelAtMs.status || '');

        // ATMI Caching
        syncField('cachedImi', panelAtMi.imi_raw ? panelAtMi.imi_raw.toFixed(2) : '');
        syncField('cachedK', panelAtMi.multiplierValue !== '-' ? String(panelAtMi.multiplierValue) : '');
        syncField('cachedAtmi', panelAtMi.recommendedIn !== 'N/A' ? String(panelAtMi.recommendedIn) : '');
        syncField('cachedAtmiStatus', panelAtMi.status || '');

        // SMSTh Caching
        if (judgment.smsth) {
            syncField('cachedSmsthSize', judgment.smsth.recommendedSize || '');
            syncField('cachedSmsthTm', judgment.smsth.tm ? String(judgment.smsth.tm) : '');
            syncField('cachedSmsthStatus', judgment.smsth.status || '');
            syncField('cachedSmsthInsulation', judgment.smsth.insulation || '');
        }
    }, [isOpen, panelAtMs, panelAtMi, judgment, projectInfo, updateProjectInfo]);
            ...
            >
                <div className="flex-grow flex flex-col p-6 relative h-full">
                    ...
                    <div className="flex-grow overflow-y-auto space-y-8 pr-2 scrollbar-hide pb-6" ref={scrollContainerRef}>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        ...
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <select
                                                value={projectInfo.installType || '노출'}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <select
                                                value={projectInfo.phase || '3Φ-4W'}
                                                onChange={(e) => {
                                                    const newPhase = e.target.value;
                                                    let newVoltage = projectInfo.voltage;
                                                    const cleanPh = String(newPhase || '').replace(/[-\s]/g, '').replace(/Ø/g, 'Φ').toUpperCase();
                                                    if (cleanPh.includes('1Φ')) newVoltage = '220V';
                                                ...
                                                style={{ textAlignLast: 'center' }}
                                            >
                                                <option value="1Φ-2W">1Φ-2W</option>
                                                <option value="3Φ-3W">3Φ-3W</option>
                                                <option value="3Φ-4W">3Φ-4W</option>
                                            </select>
                                        </div>
                                        <div className="flex flex-col gap-1">
                                                ...
                                                >
                                                    {[...new Set([...INSTALL_METHODS, 'X', projectInfo.kecMethod])].filter(Boolean).map(method => (
                                                        <option
                                                            key={method}
                                                            value={method}
                                                            disabled={getMethodDisabledOptions(projectInfo).includes(method)}
                                                            className={getMethodDisabledOptions(projectInfo).includes(method) ? 'text-gray-700' : ''}
                                                        >
                                                            {method}
                                                        </option>
                                                    ))}
                                            ...
                                            >
                                                {WIRE_TYPES.map(wire => (
                                                    <option
                                                        key={wire}
                                                        value={wire}
                                                        disabled={(projectInfo.kecMethod === 'X' || (projectInfo.kecMethod && projectInfo.kecMethod.toUpperCase().includes('X'))) && wire === 'HFIX'}
                                                        className={(projectInfo.kecMethod === 'X' || (projectInfo.kecMethod && projectInfo.kecMethod.toUpperCase().includes('X'))) && wire === 'HFIX' ? 'text-gray-700' : ''}
                                                    >
                                                        {wire}
                                                    </option>
                                                ))}
                        ...
                        <section className="space-y-4" ref={el => sectionRefs.current['at_b'] = el}>
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2 ml-auto">
                                    <span className="text-[10px] text-white bg-gray-900 px-1.5 py-0.5 rounded border border-gray-800">
                                        {(judgment.at_b?.phase && judgment.at_b.phase.includes('Φ')) ? judgment.at_b.phase.split('Φ')[0] + 'Φ' : (judgment.at_b?.phase || '-')}
                                    </span>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                >
                                    ...
                                    <div className="text-[13px] sm:text-[15px] font-mono text-blue-400 font-bold group-hover:text-blue-300">{judgment.at_b?.recommendedIn || '-'}</div>
                                </div>
                            </div>
                        ...
                        {/* AT_MS Section */}
                        <section className="space-y-4" ref={el => sectionRefs.current['at_ms'] = el}>
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2 ml-auto">
                                    <span className={`text-[11px] px-2 py-1 rounded font-bold ${panelAtMs.status === 'Ok' ? 'bg-green-500/20 text-green-400' : panelAtMs.status === 'Fail' ? 'bg-red-500/20 text-red-400' : 'bg-gray-500/20 text-gray-400'}`}>
                                        {panelAtMs.status}
                                    </span>
                                </div>
                            </div>
                            <div className="grid grid-cols-3 gap-3">
                                <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                    ...
                                    <div className="text-[13px] sm:text-[15px] font-mono text-white">{panelAtMs.delta}</div>
                                </div>
                                <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                    <div className="text-[9px] sm:text-[11px] text-gray-300 mb-1 leading-tight">(I<sub>MS</sub> * α) / δ<br />(최소정격)</div>
                                    <div className="text-[13px] sm:text-[15px] font-mono text-white">{panelAtMs.imsOverDelta > 0 ? panelAtMs.imsOverDelta.toFixed(2) : '-'}</div>
                                </div>
                                <div
                                    className="bg-gray-900/50 p-3 border border-blue-500/30 text-center cursor-pointer hover:bg-blue-500/10 transition-colors group"
                                    onClick={() => {
                                        const val = panelAtMs.recommendedIn;
                                        if (val && val !== 'N/A' && val !== '-') {
                                            updateProjectInfo('mccbAT', val);
                                        }
                                    }}
                                >
                                    <div className="text-[9px] sm:text-[11px] text-blue-400 mb-1 leading-tight group-hover:text-blue-300">I<sub>N</sub> [A]<br />(추천 정격)</div>
                                    <div className="text-[13px] sm:text-[15px] font-mono text-blue-400 font-bold group-hover:text-blue-300">{panelAtMs.recommendedIn}</div>
                                </div>
                            </div>
                            <div className="mt-1 flex flex-col gap-0.5 px-1 leading-tight">
                                <div className="text-[10px] text-white">* 조건: I<sub>N</sub> ≥ (I<sub>MS</sub> × α) / δ</div>
                                <div className="text-[10px] text-white">* 여유계수(α): {panelAtMs.alpha}</div>
                                <div className="text-[10px] text-white">* I<sub>MS</sub> = {panelAtMs.ims_raw > 0 ? panelAtMs.ims_raw.toFixed(2) : '-'} A</div>
                            </div>
                        </section>

                        {/* AT_MI Section */}
                        <section className="space-y-4" ref={el => sectionRefs.current['at_mi'] = el}>
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2 ml-auto">
                                    <button
                                        onClick={() => {
                                            const nextType = panelMiMultiplierType === 'delta' ? 'delta2' : 'delta';
                                            setPanelMiMultiplierType(nextType);
                                            onUpdateKecSettings && onUpdateKecSettings('atMiMultiplierType', nextType);
                                        }}
                                    ...
                                    >
                                        <span>{panelMiMultiplierType === 'delta' ? 'δ' : 'δ₂'}</span>
                                        <span className="hidden sm:inline">{panelAtMi.multiplierValue}</span>
                                    </button>
                                    <span className={`text-[11px] px-2 py-1 rounded font-bold ${panelAtMi.status === 'Ok' ? 'bg-green-500/20 text-green-400' : panelAtMi.status === 'Fail' ? 'bg-red-500/20 text-red-400' : 'bg-gray-500/20 text-gray-400'}`}>
                                        {panelAtMi.status}
                                    </span>
                                </div>
                            </div>
                            <div className="grid grid-cols-3 gap-3">
                                <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                    <div className="text-[9px] sm:text-[11px] text-gray-300 mb-1 leading-tight">
                                        {panelMiMultiplierType === 'delta' ? 'δ' : 'δ₂'}<br />
                                        ({panelMiMultiplierType === 'delta' ? '규약동작배율' : '순시특성 배율'})
                                    </div>
                                    <div className="text-[13px] sm:text-[15px] font-mono text-white">{panelAtMi.multiplierValue}</div>
                                </div>
                                <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                    <div className="text-[9px] sm:text-[11px] text-gray-300 mb-1 leading-tight">
                                        (I<sub>MI</sub> * α) / {panelMiMultiplierType === 'delta' ? 'δ' : 'δ₂'}<br />
                                        (최소정격)
                                    </div>
                                    <div className="text-[13px] sm:text-[15px] font-mono text-white">{panelAtMi.imiOverM > 0 ? panelAtMi.imiOverM.toFixed(2) : '-'}</div>
                                </div>
                                <div
                                    className="bg-gray-900/50 p-3 border border-blue-500/30 text-center cursor-pointer hover:bg-blue-500/10 transition-colors group"
                                    onClick={() => {
                                        const val = panelAtMi.recommendedIn;
                                        if (val && val !== 'N/A' && val !== '-') {
                                            updateProjectInfo('mccbAT', val);
                                        }
                                    }}
                                >
                                    <div className="text-[9px] sm:text-[11px] text-blue-400 mb-1 leading-tight group-hover:text-blue-300">I<sub>N</sub> [A]<br />(추천 정격)</div>
                                    <div className="text-[13px] sm:text-[15px] font-mono text-blue-400 font-bold group-hover:text-blue-300">{panelAtMi.recommendedIn}</div>
                                </div>
                            </div>
                            <div className="mt-1 flex flex-col gap-0.5 px-1 leading-tight">
                                <div className="text-[10px] text-white">* 조건: I<sub>N</sub> ≥ (I<sub>MI</sub> × α) / {panelMiMultiplierType === 'delta' ? 'δ' : 'δ₂'} (단, I<sub>MI</sub> = I<sub>MS</sub> × K)</div>
                                <div className="text-[10px] text-white">* K(돌입전류 배율): {panelAtMi.k}</div>
                                <div className="text-[10px] text-white">* 여유계수(α): {panelAtMi.alpha}</div>
                                <div className="text-[10px] text-white">* I<sub>MI</sub> = {panelAtMi.imi_raw > 0 ? panelAtMi.imi_raw.toFixed(2) : '-'} A</div>
                            </div>
                        </section>
                                ...
                                <div className="flex justify-between text-[11px] sm:text-[13px] border-t border-gray-800 pt-3">
                                    ...
                                    <div className="text-right flex items-center gap-1">
                                        ...
                                        {judgment.sb?.parallelN > 1 && (
                                            <span className="text-yellow-400 font-bold text-[11px] sm:text-[13px] whitespace-nowrap">
                                                × {getCableLineCount(judgment.sb.parallelN, panelCircuit?.size, panelPhase)} <span className="text-[9px] sm:text-[11px] text-yellow-500/70">LINE</span>
                                            </span>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="bg-gray-900/50 p-4 border border-gray-800 space-y-3">
                                ...
                                <div className="flex justify-between items-center text-[11px] sm:text-[13px] pt-1">
                                    ...
                                    <div className="text-right flex items-center gap-1">
                                        ...
                                        {judgment.scb?.parallelN > 1 && (
                                            <span className="text-yellow-400 font-bold text-[11px] sm:text-[13px] whitespace-nowrap">
                                                × {getCableLineCount(judgment.scb.parallelN, panelCircuit?.size, panelPhase)} <span className="text-[9px] sm:text-[11px] text-yellow-500/70">LINE</span>
                                            </span>
                                        )}
                                    </div>
                                </div>
                                <div className="flex justify-between text-[11px] sm:text-[13px] pt-1">
                                    ...
                                    <span
                                        ...
                                        onClick={() => {
                                            const val = judgment.scb?.recommendedSize?.size || judgment.scb?.recommendedSize;
                                            if (val && val !== '-') {
                                                updateProjectInfo('cableSize', val);
                                            }
                                        }}
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                        ...
                                        <div className="text-[13px] sm:text-[15px] font-mono text-white flex items-center justify-center gap-1">
                                            {projectInfo.branchDistance || 30} <span className="text-[9px] sm:text-[10px] text-gray-600">[m]</span>
                                        </div>
                                    </div>
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                        ...
                        {judgment.smse?.status !== '-' && (
                            <section className="space-y-4" ref={el => sectionRefs.current['smse'] = el}>
                                <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex justify-between items-center text-[11px] sm:text-[13px]">
                                            ...
                                            <div className="text-right flex items-center gap-1">
                                                ...
                                                {judgment.smse?.parallelN > 1 && (
                                                    <span className="text-yellow-400 font-bold text-[11px] sm:text-[13px] whitespace-nowrap">
                                                        × {getCableLineCount(judgment.smse.parallelN, panelCircuit?.size, panelPhase)} <span className="text-[9px] sm:text-[11px] text-yellow-500/70">LINE</span>
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                        ...
                        <section className="space-y-4" ref={el => sectionRefs.current['ssc'] = el}>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    ...
                                    >
                                        <div className="text-[9px] sm:text-[11px] text-gray-300 mb-1 leading-tight group-hover:text-blue-400 transition-colors">추천 [㎟]<br />(표준 단면적)</div>
                                        <div className="text-[13px] sm:text-[15px] font-mono text-blue-400 font-bold group-hover:text-blue-300 transition-colors">{judgment.ssc?.recommendedSize || '-'}</div>
                                    </div>
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                        ...
                        {/* SMSTh Section */}
                        <section className="space-y-4" ref={el => sectionRefs.current['smsth'] = el}>
                            <div className="flex items-start gap-2">
                                ...
                                <div className="flex items-center gap-2 ml-auto">
                                    <span className={`text-[11px] px-2 py-1 rounded font-bold shrink-0 ${judgment.smsth?.status === 'Ok' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                        ...
                                        <div className="text-[13px] sm:text-[15px] font-mono text-yellow-400 font-bold">
                                            {judgment.smsth?.isParallel && (judgment.smsth?.n || 1) > 1
                                                ? `${panelCircuit?.size || '-'}x${judgment.smsth.n}`
                                                : (panelCircuit?.size || '-')}
                                        </div>
                                    </div>
                                    <div
                                    ...
                                    >
                                        <div className="text-[9px] sm:text-[11px] text-gray-300 mb-1 leading-tight group-hover:text-blue-400 transition-colors">추천 [㎟]<br />(표준 단면적)</div>
                                        <div className="text-[13px] sm:text-[15px] font-mono text-blue-400 font-bold group-hover:text-blue-300 transition-colors">{judgment.smsth?.recommendedSize || '-'}</div>
                                    </div>
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                        <div className="text-[9px] sm:text-[11px] text-gray-300 mb-1 leading-tight">I<sub>MS</sub> [A]<br />(기동 전류 합계)</div>
                                        <div className="text-[13px] sm:text-[15px] font-mono text-white">{(judgment.smsth?.ims || 0).toFixed(1)}</div>
                                    </div>
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                        <div className="text-[9px] sm:text-[11px] text-gray-300 mb-1 leading-tight">t<sub>m</sub> [s]<br />(최대 기동 시간)</div>
                                        <div className="text-[13px] sm:text-[15px] font-mono text-white">{judgment.smsth?.tm || '-'}</div>
                                    </div>
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                        <div className="text-[9px] sm:text-[11px] text-gray-300 mb-1 leading-tight">n<br />(병렬 가닥 수)</div>
                            ...
                            <div className="text-[10px] text-white px-1 leading-tight italic">
...
export default PowerPanelKECDrawer;
