                ...
                <div className="col-span-12 border border-gray-900 bg-black p-4 relative">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        <div className="bg-gray-900/30 p-4 min-h-[100px]">
                            ...
                            <div className="text-2xl font-bold text-green-400 flex items-baseline flex-wrap">
                                <span className="flex items-baseline">
                                    <input
                                        type="number"
                                        value={projectInfo.mainCapacity || ''}
                                        readOnly
                                        className="bg-transparent border-none text-green-400 font-bold text-2xl outline-none text-left [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none p-0 cursor-default"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-4">
                        <div className="bg-gray-900/30 p-4 min-h-[100px] flex flex-col justify-between">
                            <div className="text-gray-300 text-[10px] uppercase tracking-widest mb-1">System Type</div>
                            <div>
                                <div className="text-2xl font-bold text-yellow-400 flex items-baseline">
                                    <div className="relative inline-flex items-baseline">
                                        <span className="invisible whitespace-pre select-none">{projectInfo.usageType || 'S+PF'}</span>
                                        <select
                                            value={projectInfo.usageType || 'S+PF'}
                                        ...
                                        >
                                            {Number(projectInfo.mainCapacity || 0) < 1000 ? (
                                                <>
                                                    <option value="S+PF" className="bg-black">S+PF</option>
                                                    <option value="PF+C" className="bg-black">PF+C</option>
                                                </>
                                            ) : (
                                                <option value="PF+C" className="bg-black">PF+C</option>
                                            )}
                                    ...
                                    <span className="text-[12px] text-gray-400 ml-1">TYPE</span>
                                </div>
                                <div className="text-[12px] font-bold text-[#d1d5db] mt-1 space-y-0.5">
                                    <div>{Number(projectInfo.mainCapacity || 0) < 1000 ? '간이수전설비' : '정식수전설비'}</div>
                                    <div>
                                        {projectInfo.usageType === 'S+PF' ? 'Switch + P.Fuse' : 'P.Fuse + C.B'}
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="bg-gray-900/30 p-4 min-h-[100px] flex flex-col justify-between">
                            <div className="text-gray-300 text-[10px] uppercase tracking-widest mb-1">SWITCH Type</div>
                            <div className="-mt-[5px]">
                                {Number(projectInfo.mainCapacity || 0) >= 1000 ? (
                                    <>
                                        <div className="text-2xl font-bold text-white uppercase flex items-baseline">
                                            <span>LBS</span>
                                        </div>
                                        <div className="text-[12px] font-bold text-white mt-1 space-y-0.5 opacity-90">
                                            <div>3P 24kV</div>
                                            <div>630A / 20kA</div>
                                        </div>
                                    </>
                        ...
                        <div className="bg-gray-900/30 p-4 min-h-[100px]">
                            <div className="text-gray-300 text-[11px] uppercase tracking-widest mb-1.5 text-left">1차-한류형 FUSE</div>
                            <div className="flex items-baseline leading-none">
                                <div className="flex items-baseline">
                                    <span className="text-2xl font-bold text-white tracking-tight">
                                        {formatFuseRating(mofData.primaryPF).split('/')[0].replace(/[^0-9.]/g, '') || '-'}
                                    </span>
                                    {formatFuseRating(mofData.primaryPF) !== '-' && (
                                        <span className="text-[12px] text-[#d1d5db] ml-1 font-normal uppercase">A</span>
                                    )}
                                    {formatFuseRating(mofData.primaryPF).includes('/') && (
                                        <>
                                            <span className="text-2xl font-normal text-[#d1d5db] leading-none">/</span>
                                            <span className="text-2xl font-bold text-white tracking-tight">
                                                {formatFuseRating(mofData.primaryPF).split('/')[1].replace(/[^0-9.]/g, '')}
                                            </span>
