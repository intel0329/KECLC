...
const UpsKECDrawer = ({ isOpen, onClose, projectInfo, updateProjectInfo, totalLoad, kecSettings, highlightSection, powerLoads = [] }) => {
    ...
    const isCableMethodValid = (info) => {
        ...
        if (info.wire === 'HFIX') {
            if (info.kecMethod.toUpperCase().includes('X')) return false; 
    ...
    const isATViolation = useMemo(() => {
        if (!powerLoads || !projectInfo) return false;
        const globalMainAT = Number(projectInfo.mccbAT) || 0;

        return powerLoads.some(load => {
            const branchAT = Number(load.at) || 0;
            const bankMainAT = Number(load.bankMainAT) || 0;
        ...
        >
            ...
            >
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                                        ...
                                        <div className="text-[13px] sm:text-[15px] font-mono text-white">
                                            {projectInfo.branchDistance || 1.5} <span className="text-[9px] sm:text-[10px] text-gray-600 ml-0.5">[m]</span>
                                        </div>
                                    </div>
                                    <div className="bg-gray-900/50 p-3 border border-gray-800 text-center">
                ...
                <ParallelConductorPopup
                    ...
                    onApply={(method) => {
                        setIsParallelPopupOpen(false);
                        updateProjectInfo('kecMethod', method);
                    }}
                    currentMethod={previousMethod}
                    wire={projectInfo.wire}
                    cableSize={projectInfo.cableSize}
                    thresholdArea={kecSettings?.cableCondition?.area || 50}
                />
            </div>
        </div>
    );
